<?php
/** Exact PayPal P4 two-client enable/disable isolation rehearsal. */

define('RED_PAYPAL_P3_FIXTURE_ONLY', true);
require_once __DIR__ . '/paypal-payment-adapter-p3-enable-self-test.php';
require_once $projectRoot . '/includes/addon_disable_helpers.php';

$clientBDatabase = getenv('RED_PAYPAL_P4_CLIENT_B_DATABASE');
if (!is_string($clientBDatabase)
    || preg_match(
        '/\Aredcms_paypal_p4_b_[A-Za-z0-9_]+\z/',
        $clientBDatabase
    ) !== 1
    || $clientBDatabase === DBNAME
) {
    fwrite(STDERR, "PayPal P4 client B database is invalid.\n");
    exit(65);
}
if (preg_match(
    '/\Aredcms_paypal_p4_a_[A-Za-z0-9_]+\z/',
    (string) DBNAME
) !== 1) {
    fwrite(STDERR, "PayPal P4 client A database is invalid.\n");
    exit(65);
}
...
function red_paypal_p4_assert($condition, $message): void
{
    global $assertions;
    $assertions++;
    if (!$condition) {
...
function red_paypal_p4_create_owner(
    $connection,
    int $actorId,
    string $suffix
): bool {
    if (preg_match('/\A[ab]\z/D', $suffix) !== 1) {
        return false;
    }
    $password = password_hash(
        'PayPalP4-Disposable-' . strtoupper($suffix) . '-2026!',
        PASSWORD_DEFAULT
    );
    $username = 'codex_paypal_p4_' . $suffix;
    $alias = 'PayPalP4' . strtoupper($suffix);
    $email = 'paypal-p4-' . $suffix . '@example.test';
...
function red_paypal_p4_prepare_client(
    $connection,
    string $label,
    int $actorId,
    array $catalog,
    ...
    if (!in_array($label, ['a', 'b'], true)
        || !red_paypal_p4_create_owner($connection, $actorId, $label)
    ...
    $result['returnOrigin'] =
        'https://client-' . $label . '.example.test';
    $result['webhookId'] = 'WH-TEST-PAYPAL-P4-' . strtoupper($label);
    $result['secretReferences'] = [
        'paypal.client-id' =>
            'config:p4-' . $label . '-paypal-client-id',
        'paypal.client-secret' =>
            'config:p4-' . $label . '-paypal-client-secret',
    ];
    if (!red_paypal_p3_store_settings(
        $connection,
        'redcms.store-lite-paypal',
        $actorId,
        $result['returnOrigin'],
...
function red_paypal_p4_state_fingerprint($connection): string
{
    $queries = [
    ...
    foreach ($queries as $query) {
        ...
        if (!$queryResult) {
            throw new RuntimeException('Could not fingerprint PayPal P4 client.');
        }
        $rows = [];
...
function red_paypal_p4_load_order($connection, array $catalog): ?array
{
    $enabledIds = [];
...
try {
    ...
    $clientAState = red_paypal_p4_prepare_client(
        $connection,
        'a',
        $actorId,
        $catalog,
    ...
    $clientBState = red_paypal_p4_prepare_client(
        $connectionB,
        'b',
        $actorId,
        $catalog,
    ...
    red_paypal_p4_assert(
        ...
        'each client independently installs, configures, and enables PayPal'
    );
    red_paypal_p4_assert(
        !hash_equals(
    ...
    red_paypal_p4_assert(
        red_paypal_p2_scalar(
            ...
            "SELECT COUNT(*) FROM RED_Addon_Settings
             WHERE PackageID='redcms.store-lite-paypal'
               AND (ValueJSON LIKE '%client-b%'
                    OR ValueJSON LIKE '%P4-B%'
                    OR SecretReference LIKE '%p4-b%')"
        ) === '0'
            && red_paypal_p2_scalar(
                $connectionB,
                "SELECT COUNT(*) FROM RED_Addon_Settings
                 WHERE PackageID='redcms.store-lite-paypal'
                   AND (ValueJSON LIKE '%client-a%'
                        OR ValueJSON LIKE '%P4-A%'
                        OR SecretReference LIKE '%p4-a%')"
            ) === '0',
        'neither client contains the other client setting or reference'
    );

    $orderA = red_paypal_p4_load_order($connection, $catalog);
    $orderB = red_paypal_p4_load_order($connectionB, $catalog);
    red_paypal_p4_assert(
        $orderA === ['redcms.store-lite', 'redcms.store-lite-paypal']
            && $orderB === $orderA,
        'both enabled registries share declarative dependency order'
    );
    ...
    red_paypal_p4_assert(
        ...
        'client A receives one exact non-executing PayPal disable plan'
    );
    $beforeRollbackA = red_paypal_p4_state_fingerprint($connection);
    $beforeRollbackB = red_paypal_p4_state_fingerprint($connectionB);
    $rollbackA = red_addon_disable_package(
        ...
        static function (): void {
            throw new RuntimeException('forced_paypal_p4_disable_failure');
        }
    );
    red_paypal_p4_assert(
        $rollbackA['status'] === 'disable_transaction_failed'
            && hash_equals(
                $beforeRollbackA,
                red_paypal_p4_state_fingerprint($connection)
            )
            && hash_equals(
                $beforeRollbackB,
                red_paypal_p4_state_fingerprint($connectionB)
            ),
        'forced client A disable rolls back A and leaves B unchanged'
    );
    ...
    red_paypal_p4_assert(
        ...
        'client A disables atomically while every client B fact stays unchanged'
    );
    red_paypal_p4_assert(
        red_paypal_p2_scalar(
            $connection,
        ...
        'disable retains A settings/tables while B stays independently enabled'
    );
    red_paypal_p4_assert(
        red_paypal_p4_load_order($connection, $catalog)
            === ['redcms.store-lite']
            && red_paypal_p4_load_order($connectionB, $catalog)
                === ['redcms.store-lite', 'redcms.store-lite-paypal'],
        'runtime order excludes disabled A but retains enabled B'
    );
    ...
    red_paypal_p4_assert(
        ...
        'repeat client A disable refuses without affecting B'
    );
    ...
    red_paypal_p4_assert(
        is_string($encodedEvidence)
            && !str_contains($encodedEvidence, 'client-a.example.test')
            && !str_contains($encodedEvidence, 'client-b.example.test')
            && !str_contains($encodedEvidence, 'config:p4-'),
        'bounded evidence contains hashes but no setting or reference value'
    );

    echo 'PayPal P4 two-client isolation self-test passed: '
