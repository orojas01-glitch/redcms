<?php
/**
 * Server-local Owner-authorized atomic add-on enable transition.
 */
...
function red_addon_enable_cli_usage()
{
    fwrite(
        STDERR,
        "Usage:\n" .
...
foreach (array_slice($argv, 1) as $argument) {
    if ($argument === '--apply') {
        ...
        $options['confirmVersion'] = substr($argument, strlen('--confirm-version='));
        ...
        red_addon_enable_cli_usage();
        exit(64);
    }
}
if (!red_addon_valid_package_id($options['package']) || $options['actorAdmin'] <= 0) {
    red_addon_enable_cli_usage();
    exit(64);
}
...
if (!red_addon_install_storage_available($connection)) {
    fwrite(STDERR, "Add-on enablement storage is unavailable. Apply pending core migrations first.\n");
    $db->close();
    exit(65);
}
...
if (empty($plan['valid'])) {
    fwrite(STDERR, 'Enablement plan refused: ' . implode(', ', $plan['errors']) . PHP_EOL);
    $db->close();
    exit(65);
}
...
printf("Activation profile: %s\n", (string) ($plan['activationProfile']['id'] ?? 'unavailable'));
echo 'Registrar validation: required before state change' . PHP_EOL;
echo 'Plan SHA-256: ' . $plan['planSha256'] . PHP_EOL;

if (!$options['apply']) {
    echo "DRY RUN: no database changes or package code execution occurred.\n";
    echo "Test the fixed registrar on a disposable restored database and retain backup evidence.\n";
    echo "Provide exact target, plan, backup, and installed-disabled confirmations with --apply.\n";
    $db->close();
    exit(0);
}
...
if ($options['confirmDatabase'] !== $plan['database']
    ...
    fwrite(STDERR, "Apply requires exact database, package, version, plan, nonzero backup checksum, and installed-disabled-state confirmations.\n");
    $db->close();
    exit(64);
}

$result = red_addon_enable_package(
    $connection,
    $options['package'],
    $projectRoot,
    $options['actorAdmin'],
...
if ($result['status'] !== 'enabled') {
    fwrite(STDERR, 'Enablement failed closed: ' . $result['status'] . PHP_EOL);
    exit(1);
}
printf(
    "Enabled %s %s after fixed registrar validation; request bootstrap may load it on a later request.\n",
    $result['packageId'],
    $result['version']
);
exit(0);
