<?php
/** Pure P3E-9C3B1 operator-command source contract. */

if (PHP_SAPI !== 'cli') {
    http_response_code(404);
    exit;
}

$projectRoot = dirname(__DIR__);
$commandPath = $projectRoot
    . '/scripts/admin-sandbox-checkout-transport-double-execute.php';
$source = is_file($commandPath) ? (string) file_get_contents($commandPath) : '';
$assertions = 0;

function red_addon_checkout_transport_operator_assert($condition, $message)
{
    global $assertions;
    $assertions++;
    if (!$condition) {
        throw new RuntimeException('Assertion failed: ' . $message);
    }
}

try {
    red_addon_checkout_transport_operator_assert(
        $source !== ''
            && str_contains($source, "PHP_SAPI !== 'cli'")
            && str_contains(
                $source,
                'addon_sandbox_checkout_mutation_transport_double_helpers.php'
            ),
        'command is CLI-only and loads only the reviewed C3A runner'
    );
    foreach ([
        '--actor-admin=', '--evidence-file=', '--confirm-database=',
        '--confirm-package=', '--confirm-version=', '--confirm-state=',
        '--confirm-plan-sha256=', '--confirm-input-sha256=',
        '--confirm-authorization-sha256=',
        '--confirm-authorization-state-sha256=',
        '--confirm-claim-state-sha256=',
        '--confirm-execution-start-sha256=', '--confirm-backup-sha256=',
        '--confirm-operation=', '--confirm-target=',
        '--confirm-maximum-attempts=', '--confirm-retry-authorized=',
        '--confirm-network-authorized=',
        '--confirm-provider-mutation-authorized=',
        '--confirm-checkout-creation-authorized=', '--apply',
    ] as $confirmation) {
        red_addon_checkout_transport_operator_assert(
            str_contains($source, $confirmation),
            $confirmation . ' is required by the operator contract'
        );
    }
    red_addon_checkout_transport_operator_assert(
        str_contains($source, "=== '0.1.5'")
            && str_contains($source, "=== 'enabled'")
            && str_contains(
                $source,
                "'checkout.create-sandbox-transport-double'"
            )
            && str_contains($source, "'core-in-memory-transport-double'"),
        'apply is pinned to exact package, state, operation, and target'
    );
    red_addon_checkout_transport_operator_assert(
        str_contains($source, "=== '1'")
            && substr_count($source, "=== 'no'") >= 4
            && str_contains($source, "str_repeat('0', 64)"),
        'one attempt, no effects, and a nonzero backup are explicit'
    );
    red_addon_checkout_transport_operator_assert(
        substr_count(
            $source,
            'red_addon_checkout_mutation_transport_plan('
        ) === 1
            && substr_count(
                $source,
                'red_addon_checkout_mutation_execute_transport_double('
            ) === 1
            && substr_count(
                $source,
                'new RED_Addon_Checkout_Mutation_Transport_Double('
            ) === 1,
        'command plans once, constructs one final double, and calls once'
    );
    red_addon_checkout_transport_operator_assert(
        str_contains($source, "if (!\$options['apply'])")
            && str_contains($source, 'DRY RUN:')
            && str_contains($source, 'no double ran')
            && str_contains($source, 'no network or provider contact occurred'),
        'default mode is an explicit non-executing dry run'
    );
    red_addon_checkout_transport_operator_assert(
        str_contains($source, "=== 'transport_double_completed'")
            && str_contains($source, 'The attempt remains consumed')
            && str_contains($source, 'no retry is authorized'),
        'only the exact bounded result succeeds and every failure burns'
    );
    red_addon_checkout_transport_operator_assert(
        !str_contains($source, '--secret-key=')
            && !str_contains($source, '--credential=')
            && !str_contains($source, 'RED_ADDON_SECRET_VALUES_JSON')
            && !str_contains($source, 'sk_test_')
            && !str_contains($source, 'sk_live_')
            && !str_contains($source, 'rk_test_')
            && !str_contains($source, 'rk_live_'),
        'command accepts and contains no credential value or literal'
    );
    foreach ([
        'curl_', 'fsockopen(', 'stream_socket_client(', 'socket_',
        'api.stripe.com', 'Authorization:', 'php://input', '$_POST',
        'shell_exec(', 'exec(', 'passthru(', 'sleep(', 'usleep(',
        'red_addon_secret_resolve', 'red_addon_runtime_register_package',
        'red_addon_adapter_invoke_registered',
    ] as $forbidden) {
        red_addon_checkout_transport_operator_assert(
            !str_contains($source, $forbidden),
            $forbidden . ' is absent from the operator command'
        );
    }
    red_addon_checkout_transport_operator_assert(
        !file_exists(
            $projectRoot . '/admin/bin/sandbox-checkout-transport-execute.php'
        )
            && !file_exists(
                $projectRoot . '/bin/sandbox-checkout-transport-execute.php'
            )
            && !file_exists(
                $projectRoot . '/admin/sandbox-checkout-transport-execute.php'
            ),
        'no browser or public command bridge exists'
    );

    echo 'Sandbox Checkout P3E-9C3B1 operator command self-test passed: '
        . $assertions . " assertions.\n";
} catch (Throwable $throwable) {
    fwrite(STDERR, $throwable->getMessage() . "\n");
    exit(1);
}

?>
