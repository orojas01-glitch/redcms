...
EXPECTED_ADAPTER_COMMIT="44ed7b3bd8f84f3f24340a6afc39881e8dee8c5d"
...
REHEARSAL_DATABASE="${RED_CHECKOUT_REAL_POST_DATABASE:-redcms_acceptance_d4c2_$(date +%s)_$$}"
...
cleanup() {
    ...
    if [[ "$RUN_SUCCEEDED" -eq 1 && "$cleanup_status" -eq 0 ]]; then
        echo 'P3E-9D4C2 cleanup passed: database:0 grant:0 staged-project:0 evidence:0 environment:clear source-repositories:unchanged primary:unchanged'
...
if [[ ! "$REHEARSAL_DATABASE" =~ ^redcms_acceptance_d4c2_[A-Za-z0-9_]+$
    || ${#REHEARSAL_DATABASE} -gt 56
    || "$REHEARSAL_DATABASE" == "$RED_DB_NAME_RESOLVED"
]]; then
    echo "Unsafe D4C2 database: $REHEARSAL_DATABASE" >&2
    exit 64
fi
if [[ ! -x "$FRANKENPHP_BIN"
    || ! -d "$ADAPTER_ROOT/.git"
...
RUN_SUCCEEDED=1
echo "P3E-9D4C2 no-contact rehearsal passed: core:$CORE_COMMIT adapter:$EXPECTED_ADAPTER_COMMIT store-lite:$EXPECTED_STORE_COMMIT dry-run:1 incomplete-apply-refused:1 changed-confirmation-refused:1 real-apply:0 start-result:0 provider-effects:0"
