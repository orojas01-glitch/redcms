...
try {
    ...
    red_clean_starter_test_assert(
        str_contains($installer, 'CREATE TABLE `RED_Addon_Installations`')
            ...
            && !preg_match(
                '/INSERT\\s+INTO\\s+`?RED_Addon_(?:Installations|Migrations|Activity_Log|Settings|Admin_Action_Executions|Public_Mutation_Subjects|Public_Mutation_CSRF_Tokens|Public_Mutation_Rate_Limits|Public_Mutation_Idempotency_Keys|Public_Mutation_Executions)`?/i',
                $installer
            ),
        'starter ships empty add-on registry, settings, public-mutation token/rate/idempotency/replay storage, action-execution storage, and full component-id capacity with no package, setting, migration, lifecycle, or business state'
    );
