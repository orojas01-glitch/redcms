...
if [ -n "${PHP_CLI:-}" ]; then
  ...
  exec "$PHP_CLI" "$SCRIPT_DIR/theme-preview-selected-contact-self-test.php"
fi

if [ ! -x "$FRANKENPHP_BIN" ]; then
  echo "FrankenPHP CLI not found or not executable: $FRANKENPHP_BIN" >&2
  echo "Set FRANKENPHP_BIN=/path/to/frankenphp or PHP_CLI=/path/to/php." >&2
  exit 1
fi

exec "$FRANKENPHP_BIN" php-cli "$SCRIPT_DIR/theme-preview-selected-contact-self-test.php"
