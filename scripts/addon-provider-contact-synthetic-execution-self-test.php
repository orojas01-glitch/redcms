<?php
/**
 * Disposable-database P3E-8B3B synthetic execution acceptance fixture.
 */
...
$apiReference = 'config:p3e8b3b-stripe-secret-key';
$webhookReference = 'config:p3e8b3b-stripe-webhook-secret';
$GLOBALS['RED_P3E8B3B_REGISTRAR_CALLS'] = 0;
$GLOBALS['RED_P3E8B3B_SYNTHETIC_DOUBLE'] = null;
...
function red_addon_provider_contact_execution_test_write_adapter(
    ...
    $entrypoint = <<<'PHP'
<?php
$GLOBALS['RED_P3E8B3B_REGISTRAR_CALLS'] =
    (int) ($GLOBALS['RED_P3E8B3B_REGISTRAR_CALLS'] ?? 0) + 1;
return static function ($registry): void {
    $registry->registerAdapter(
        ...
        static function (
            ...
            $double = $GLOBALS['RED_P3E8B3B_SYNTHETIC_DOUBLE'] ?? null;
            if (!is_callable($double)) {
                $apiKey = null;
                return RED_Addon_Adapter_Result::failure(
                    'synthetic_double_unavailable'
                );
            }
            try {
                $outcome = $double(
...
function red_addon_provider_contact_synthetic_test_readiness()
{
    $readiness = red_addon_provider_contact_test_readiness();
    $readiness['contactPlan']['packageVersion'] = '0.1.3';
    $readiness['contactPlan']['runtimeProviderTransport'] = 'synthetic_only';
...
try {
    ...
    $readiness = red_addon_provider_contact_synthetic_test_readiness();
    ...
    $plan = red_addon_provider_contact_synthetic_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    red_addon_provider_contact_execution_test_assert(
        ...
        'historical loopback runner refuses the synthetic-only profile'
    );
    red_addon_provider_contact_execution_test_assert(
    ...
    $keyMatched = false;
    $planMatched = false;
    $GLOBALS['RED_P3E8B3B_SYNTHETIC_DOUBLE'] = static function (
        array $contactPlan,
        string $apiKey
    ) use (
        &$syntheticCalls,
    ...
    $executed = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    red_addon_provider_contact_execution_test_assert(
        ...
        'one claimed execution reaches only the contained synthetic adapter'
    );
    red_addon_provider_contact_execution_test_assert(
        $syntheticCalls === 1
            && $keyMatched
            && $planMatched
            && $GLOBALS['RED_P3E8B3B_REGISTRAR_CALLS'] === 1
            && !str_contains(json_encode($executed), $syntheticKey)
            && ($executed['boundedOutcome']['responseBodyIncluded'] ?? null)
                === false,
        'synthetic receives exact plan and scoped key without disclosure'
    );
    ...
    red_addon_provider_contact_execution_test_assert(
        red_addon_provider_contact_execution_test_scalar(
            ...
            "SELECT CONCAT_WS(':',
                ...
                (SELECT COUNT(*) FROM RED_Addon_Activity_Log
                 WHERE PackageID='$adapterPackageId'
                   AND DetailCode='provider_contact_synthetic_resource_miss'))"
        ) === '1:1',
        'bounded start and synthetic outcome audits are committed'
    );
    $replay = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $secondPlan = red_addon_provider_contact_synthetic_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    putenv('RED_ADDON_SECRET_VALUES_JSON');
    $missingSecret = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $missingReplay = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $thirdPlan = red_addon_provider_contact_synthetic_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    $startAuditFailure = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    red_addon_provider_contact_execution_test_assert(
        ($startAuditFailure['status'] ?? '')
            ...
            && $syntheticCalls === 1
            && $GLOBALS['RED_P3E8B3B_REGISTRAR_CALLS'] === 2,
        'failed start audit rolls back before registrar or synthetic execution'
    );
    $afterStartRollback = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $fourthPlan = red_addon_provider_contact_synthetic_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    $outcomeAuditFailure = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $afterOutcomeFailure = red_addon_provider_contact_execute_synthetic(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $missingClaimPlan = red_addon_provider_contact_synthetic_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    foreach ([
        ...
        red_addon_provider_contact_execution_test_assert(
            ...
            $forbidden . ' is absent from core synthetic execution source'
        );
    }
    red_addon_provider_contact_execution_test_assert(
        red_addon_provider_contact_execution_test_scalar(
        ...
        'synthetic orchestration creates no package or public-mutation row'
    );
    ...
    echo 'Provider contact P3E-8B3B synthetic execution self-test passed: ' .
