<?php
/**
 * Disposable-database P3E-8B3C2 provider execution acceptance fixture.
 */
...
require_once $projectRoot .
    '/includes/addon_provider_contact_synthetic_execution_helpers.php';

$assertions = 0;
$actorId = 2147000992;
$adapterPackageId = 'redcms.store-lite-stripe-checkout';
$storePackageId = 'redcms.store-lite';
$tableName = 'RED_Addon_Stripe_Provider_Execution_Fixture';
...
$apiReference = 'config:p3e8b3c2-stripe-secret-key';
$webhookReference = 'config:p3e8b3c2-stripe-webhook-secret';
$GLOBALS['RED_P3E8B3C2_REGISTRAR_CALLS'] = 0;
$GLOBALS['RED_P3E8B3C2_PROVIDER_DOUBLE'] = null;
...
function red_addon_provider_contact_execution_test_write_adapter(
    ...
    $entrypoint = <<<'PHP'
<?php
$GLOBALS['RED_P3E8B3C2_REGISTRAR_CALLS'] =
    (int) ($GLOBALS['RED_P3E8B3C2_REGISTRAR_CALLS'] ?? 0) + 1;
return static function ($registry): void {
    $registry->registerAdapter(
        ...
        static function (
            ...
            $double = $GLOBALS['RED_P3E8B3C2_PROVIDER_DOUBLE'] ?? null;
            if (!is_callable($double)) {
                $apiKey = null;
                return RED_Addon_Adapter_Result::failure(
                    'provider_double_unavailable'
                );
            }
            try {
                $outcome = $double(
...
function red_addon_provider_contact_sandbox_test_readiness()
{
    $readiness = red_addon_provider_contact_test_readiness();
    $readiness['contactPlan']['packageVersion'] = '0.1.4';
    $readiness['contactPlan']['runtimeProviderTransport'] = 'provider_read_only';
    $encoded = red_addon_provider_contact_encode(
        $readiness['contactPlan']
    );
    $readiness['planSha256'] = is_string($encoded)
        ? hash('sha256', $encoded)
        : '';
    return $readiness;
}
...
try {
    ...
    $readiness = red_addon_provider_contact_sandbox_test_readiness();
    $wrongMode = $readiness;
    $wrongMode['contactPlan']['runtimeProviderTransport'] = 'synthetic_only';
    $wrongModeEncoded = red_addon_provider_contact_encode(
        $wrongMode['contactPlan']
    );
    $wrongMode['planSha256'] = is_string($wrongModeEncoded)
        ? hash('sha256', $wrongModeEncoded)
        : '';
    $wrongVersion = $readiness;
    $wrongVersion['contactPlan']['packageVersion'] = '0.1.3';
    $wrongVersionEncoded = red_addon_provider_contact_encode(
        $wrongVersion['contactPlan']
    );
    $wrongVersion['planSha256'] = is_string($wrongVersionEncoded)
        ? hash('sha256', $wrongVersionEncoded)
        : '';
    red_addon_provider_contact_execution_test_assert(
        red_addon_provider_contact_readiness_valid($readiness)
            && !red_addon_provider_contact_readiness_valid($wrongMode)
            && !red_addon_provider_contact_readiness_valid($wrongVersion),
        'shared evidence accepts only exact 0.1.4 provider-read-only profile'
    );
    ...
    $wrongRunnerPlan = red_addon_provider_contact_loopback_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    red_addon_provider_contact_execution_test_assert(
        ($wrongRunnerPlan['status'] ?? '') === 'loopback_profile_refused'
            && empty($wrongRunnerPlan['ready'])
            && empty($wrongRunnerPlan['executionStartAvailable']),
        'historical loopback runner refuses the provider-read-only profile'
    );
    $wrongSyntheticPlan =
        red_addon_provider_contact_synthetic_execution_plan(
            $connection,
            $adapterPackage,
            $catalog,
            $actorId,
    ...
    red_addon_provider_contact_execution_test_assert(
        ($wrongSyntheticPlan['status'] ?? '') === 'synthetic_profile_refused'
            && empty($wrongSyntheticPlan['ready'])
            && empty($wrongSyntheticPlan['executionStartAvailable']),
        'synthetic runner refuses the provider-read-only profile'
    );
    red_addon_provider_contact_execution_test_assert(
    ...
    $providerKey = 'rk_' . 'test_' . str_repeat('a', 32);
    $providerCalls = 0;
    $keyMatched = false;
    $planMatched = false;
    $GLOBALS['RED_P3E8B3C2_PROVIDER_DOUBLE'] = static function (
        array $contactPlan,
        string $apiKey
    ) use (
        &$providerCalls,
        ...
        $providerCalls++;
        $keyMatched = hash_equals($providerKey, $apiKey);
    ...
    putenv('RED_ADDON_SECRET_VALUES_JSON=' . json_encode([
        $apiReference => $providerKey,
    ], JSON_UNESCAPED_SLASHES));
    $executed = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    red_addon_provider_contact_execution_test_assert(
        ...
        'one claimed execution reaches only the contained provider adapter'
    );
    red_addon_provider_contact_execution_test_assert(
        $providerCalls === 1
            && $keyMatched
            && $planMatched
            && $GLOBALS['RED_P3E8B3C2_REGISTRAR_CALLS'] === 1
            && !str_contains(json_encode($executed), $providerKey)
            && ($executed['boundedOutcome']['responseBodyIncluded'] ?? null)
                === false,
        'provider receives exact plan and scoped key without disclosure'
    );
    ...
    red_addon_provider_contact_execution_test_assert(
        red_addon_provider_contact_execution_test_scalar(
            ...
            "SELECT CONCAT_WS(':',
                ...
                (SELECT COUNT(*) FROM RED_Addon_Activity_Log
                 WHERE PackageID='$adapterPackageId'
                   AND DetailCode='provider_contact_sandbox_resource_miss'))"
        ) === '1:1',
        'bounded start and provider outcome audits are committed'
    );
    $replay = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $secondPlan = red_addon_provider_contact_sandbox_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    putenv('RED_ADDON_SECRET_VALUES_JSON');
    $missingSecret = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $missingReplay = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $thirdPlan = red_addon_provider_contact_sandbox_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    $startAuditFailure = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    red_addon_provider_contact_execution_test_assert(
        ($startAuditFailure['status'] ?? '')
            ...
            && $providerCalls === 1
            && $GLOBALS['RED_P3E8B3C2_REGISTRAR_CALLS'] === 2,
        'failed start audit rolls back before registrar or provider execution'
    );
    $afterStartRollback = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $fourthPlan = red_addon_provider_contact_sandbox_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    $outcomeAuditFailure = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $afterOutcomeFailure = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $fifth = red_addon_provider_contact_execution_test_claim(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    $fifthPlan = red_addon_provider_contact_sandbox_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    $GLOBALS['RED_P3E8B3C2_PROVIDER_DOUBLE'] = static function () use (
        &$providerCalls
    ): array {
        $providerCalls++;
        return ['valid' => true];
    };
    $malformed = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
        $fifth['prepared'],
        $fifthPlan['authorizationSha256'],
        $fifthPlan['claimStateSha256'],
        $fifthPlan['executionStartStateSha256'],
        '2026-08-17T12:07:00Z'
    );
    red_addon_provider_contact_execution_test_assert(
        ($malformed['status'] ?? '') === 'indeterminate'
            && !empty($malformed['adapterInvoked'])
            && !empty($malformed['executionPerformed'])
            && !empty($malformed['networkAccess'])
            && !empty($malformed['providerContact'])
            && !empty($malformed['outcomeRecorded'])
            && $providerCalls === 4,
        'malformed post-invocation output is conservatively indeterminate'
    );
    $malformedReplay = red_addon_provider_contact_execute_sandbox(
        $connection,
        $fixtureProject,
        $actorId,
        $readiness,
    ...
    red_addon_provider_contact_execution_test_assert(
        ($malformedReplay['status'] ?? '') === 'execution_already_started'
            && $providerCalls === 4,
        'malformed post-invocation output cannot be retried'
    );
    ...
    $missingClaimPlan = red_addon_provider_contact_sandbox_execution_plan(
        $connection,
        $adapterPackage,
        $catalog,
        $actorId,
    ...
    foreach ([
        ...
        red_addon_provider_contact_execution_test_assert(
            ...
            $forbidden . ' is absent from core provider execution source'
        );
    }
    red_addon_provider_contact_execution_test_assert(
        red_addon_provider_contact_execution_test_scalar(
        ...
        'provider orchestration creates no package or public-mutation row'
    );
    ...
    echo 'Provider contact P3E-8B3C2 provider execution self-test passed: ' .
