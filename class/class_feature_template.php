		...
		if ($this->Table!='Articles'){
			...
			//echo(count($Features.'<br/>'));
				...
				for ($w=0; $w<=count($Features); $w++)
				{
					...
					switch ($Features[$w])
					{
					case 'template': //unique name

						echo('<div id="coin-template">');
						$db= new connection(DBHOST, DBUSER, DBPASS, DBNAME);
						$result = $db->query("SELECT * FROM RED_Articles WHERE Active='Y' AND Language='".language."' AND ".$this->VarFeatures." LIKE '%template%' ORDER BY ".$this->VarFeatures."_Order ASC limit 5");
						...
						while($row = mysqli_fetch_assoc($result))
							...
							if ($row['ExpDate']!='0000-00-00 00:00:00'){
								...
								if ($ActiveDate) {	
									...
									echo('<a href="'.$Link.'" target="'.$NewWindow.'"><img src="/images/articles/'.$BigPict.'" title="'.$row['Title'].'"><!--<span>'.$Title.'</span>--></a>');
			...
			//echo(count($Features.'<br/>'));
				...
				for ($w=0; $w<=count($Features); $w++)
				{
					...
					case 'template': //unique name
						...
						$db= new connection(DBHOST, DBUSER, DBPASS, DBNAME);
						if ($this->VarFeatures==='HomeFeatures')
						$result = $db->query("SELECT * FROM RED_Articles WHERE Active='Y' AND Language='".language."' AND ".$this->VarFeatures." LIKE '%template%' ORDER BY ".$this->VarFeatures."_Order ASC limit 5");
						//echo ($result->num_rows);
						$result_counter = $result->num_rows;
						while($row = mysqli_fetch_assoc($result))
