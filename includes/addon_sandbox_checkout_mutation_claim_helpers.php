<?php
/**
 * Core-owned P3E-9C2 claim for one authorized sandbox Checkout mutation.
 *
 * This boundary consumes only an exact persisted P3E-9C1 authorization. It
 * records one immutable attempt claim and one value-free audit fact. It does
 * not resolve a credential, invoke package PHP, or contact a provider.
 */

require_once __DIR__
    . '/addon_sandbox_checkout_mutation_authorization_helpers.php';

if (!function_exists('red_addon_checkout_mutation_claim_result')) {
    function red_addon_checkout_mutation_claim_result()
    {
        return [
            'valid' => false,
            'ready' => false,
...
if (!function_exists('red_addon_checkout_mutation_claim_action_id')) {
    function red_addon_checkout_mutation_claim_action_id($nonceSha256)
    {
        return red_addon_provider_contact_sha256($nonceSha256)
            ? 'sandbox-checkout-mutation-claim.' . $nonceSha256
            : '';
    }
}

if (!function_exists('red_addon_checkout_mutation_claim_execution_row')) {
    function red_addon_checkout_mutation_claim_execution_row(
        $connection,
        $packageId,
        $actionId,
        $lock = false
...
if (!function_exists('red_addon_checkout_mutation_claim_authorization_matches')) {
    function red_addon_checkout_mutation_claim_authorization_matches(
        array $row,
        ...
        return red_addon_checkout_synthetic_exact_keys($row, [
            ...
            && hash_equals(
                (string) ($authorizationPlan['syntheticPlanSha256'] ?? ''),
...
if (!function_exists('red_addon_checkout_mutation_claim_state_sha256')) {
    function red_addon_checkout_mutation_claim_state_sha256(array $plan)
    {
        foreach ([
            'packageId', 'packageVersion', 'storePackageId',
            'storePackageVersion', 'actorAdminRecordId',
        ...
        if (($plan['packageId'] ?? null)
            ...
            || ($plan['storePackageId'] ?? null) !== 'redcms.store-lite'
            || ($plan['storePackageVersion'] ?? null) !== '0.1.35'
            || (int) $plan['actorAdminRecordId'] <= 0
            || !red_addon_provider_contact_sha256($plan['ownerSubjectSha256'])
            || !red_addon_provider_contact_sha256($plan['syntheticPlanSha256'])
            || !red_addon_provider_contact_sha256($plan['inputSha256'])
            || !red_addon_provider_contact_sha256($plan['authorizationSha256'])
            ...
            || !red_addon_provider_contact_utc($plan['issuedAtUtc'])
                instanceof DateTimeImmutable
            || !red_addon_provider_contact_utc($plan['expiresAtUtc'])
                instanceof DateTimeImmutable
        ...
        $stateMaterial = [
            'schema' => 1,
            'purpose' => 'sandbox-checkout-mutation-attempt-claim',
            ...
            'authorizationNonceSha256' => $plan['authorizationNonceSha256'],
...
if (!function_exists('red_addon_checkout_mutation_claim_plan')) {
    function red_addon_checkout_mutation_claim_plan(
        $connection,
        array $package,
        array $catalog,
        $actorAdminRecordId,
        ...
        foreach ([
            'packageId', 'packageVersion', 'storePackageId',
            'storePackageVersion', 'lifecycleState',
            'actorAdminRecordId', 'ownerSubjectSha256',
            'syntheticPlanSha256', 'inputSha256',
            'authorizationSha256', 'authorizationNonceSha256',
            'authorizationStateSha256', 'issuedAtUtc', 'expiresAtUtc',
        ...
        $authorizationRow = red_addon_checkout_mutation_claim_execution_row(
            $connection,
            $result['packageId'],
            $result['authorizationActionId'],
            $lockRows
...
if (!function_exists('red_addon_checkout_mutation_claim_reserve')) {
    function red_addon_checkout_mutation_claim_reserve(
        $connection,
        array $plan
    ) {
...
if (!function_exists('red_addon_checkout_mutation_claim_audit_record')) {
    function red_addon_checkout_mutation_claim_audit_record(
        $connection,
        array $plan
    ) {
...
if (!function_exists('red_addon_checkout_mutation_claim')) {
    function red_addon_checkout_mutation_claim(
        $connection,
        $projectRoot,
        $actorAdminRecordId,
        array $input,
        ...
        $auditRecorder = $auditRecorder
            ?? 'red_addon_checkout_mutation_claim_audit_record';
        ...
        try {
            ...
            try {
                ...
                $lockedPlan = red_addon_checkout_mutation_claim_plan(
                    $connection,
                    $package,
                    $catalog,
                    $actorAdminRecordId,
                ...
                $reservation = red_addon_checkout_mutation_claim_reserve(
                    $connection,
                    $lockedPlan
                );
