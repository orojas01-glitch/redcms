<?php
/**
 * Core-owned P3E-8B3B synthetic package execution boundary.
 *
 ...
 * P3E-8B2. It accepts only adapter 0.1.3 with the synthetic_only profile and
 * contains no network or provider transport primitive.
 */

require_once __DIR__ . '/addon_provider_contact_execution_helpers.php';

if (!function_exists('red_addon_provider_contact_synthetic_start_sha256')) {
    function red_addon_provider_contact_synthetic_start_sha256(array $plan)
    {
        foreach ([
        ...
        $material = [
            'schema' => 1,
            'purpose' => 'provider-contact-synthetic-execution-start',
...
if (!function_exists('red_addon_provider_contact_synthetic_execution_plan')) {
    function red_addon_provider_contact_synthetic_execution_plan(
        $connection,
        array $package,
        array $catalog,
        $actorAdminRecordId,
        ...
        if (!is_array($contactPlan)
            || ($contactPlan['packageVersion'] ?? null) !== '0.1.3'
            || ($contactPlan['runtimeProviderTransport'] ?? null)
                !== 'synthetic_only'
        ) {
            $result['valid'] = false;
            $result['ready'] = false;
            $result['status'] = 'synthetic_profile_refused';
            $result['executionStartAvailable'] = false;
            $result['executionStartStateSha256'] = '';
            $result['errors'][] = 'synthetic_profile_refused';
            return $result;
        }
        $result['executionStartStateSha256'] =
            red_addon_provider_contact_synthetic_start_sha256($result);
        if (!red_addon_provider_contact_sha256(
            $result['executionStartStateSha256']
        )) {
            $result['valid'] = false;
            $result['ready'] = false;
            $result['status'] = 'synthetic_start_state_encoding_failed';
            $result['executionStartAvailable'] = false;
            $result['errors'][] = 'synthetic_start_state_encoding_failed';
        }
        return $result;
    }
}

if (!function_exists('red_addon_provider_contact_synthetic_outcome')) {
    function red_addon_provider_contact_synthetic_outcome(array $invocation)
    {
        $indeterminate = [
            'valid' => true,
            'contactTarget' => 'synthetic-package',
            ...
            'errors' => ['synthetic_execution_indeterminate'],
        ];
        ...
        if (empty($invocation['invoked'])
            ...
            || ($data['contactTarget'] ?? null) !== 'synthetic-package'
...
if (!function_exists('red_addon_provider_contact_synthetic_outcome_sha256')) {
    function red_addon_provider_contact_synthetic_outcome_sha256(
        array $plan,
        array $outcome
    ) {
        $material = [
            'schema' => 1,
            'purpose' => 'provider-contact-synthetic-outcome',
            'packageId' => $plan['packageId'] ?? '',
            'packageVersion' => $plan['packageVersion'] ?? '',
...
if (!function_exists('red_addon_provider_contact_synthetic_outcome_audit')) {
    function red_addon_provider_contact_synthetic_outcome_audit(
        $connection,
        array $plan,
        array $outcome
    ) {
        $detail = $outcome['outcome'] === 'resource_miss_observed'
            ? 'provider_contact_synthetic_resource_miss'
            : 'provider_contact_synthetic_' . $outcome['outcome'];
...
if (!function_exists('red_addon_provider_contact_execute_synthetic')) {
    function red_addon_provider_contact_execute_synthetic(
        $connection,
        $projectRoot,
        $actorAdminRecordId,
        array $readiness,
        ...
        $outcomeAuditRecorder = $outcomeAuditRecorder
            ?? 'red_addon_provider_contact_synthetic_outcome_audit';
        ...
        try {
            ...
            $outcome = red_addon_provider_contact_synthetic_outcome(
            ...
            $result['outcomeStateSha256'] =
                red_addon_provider_contact_synthetic_outcome_sha256(
                    $plan,
                    $outcome
                );
            if (!red_addon_provider_contact_sha256(
