...
| --- | --- | --- | --- |
| Stripe Checkout | `redcms.store-lite-stripe-checkout` `0.1.19` candidate | Adapter `0.1.18` completed hosted subscription activation/cancellation in the isolated Stripe Sandbox. The `0.1.19` candidate additionally binds one Store Lite offer to an existing non-live Stripe Product and Price, with offline contract and sealed-operation verification. | One client-specific catalog-Price Sandbox Checkout and signed lifecycle, general live-mode release, client-owned production credentials, refunds/disputes, and ACH delayed-payment lifecycle. |
