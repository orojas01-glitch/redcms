...
| Latest completed slice | Core-owned non-executing deployment review, isolated HTTPS Docker/FrankenPHP rehearsal, and the full disposable acceptance suite against a separately built current-schema baseline. The evidence validates pinned Caddy/FrankenPHP/TLS/proxy facts, artifact hashes outside the starter, process-environment trust and key rotation, desktop/mobile browser behavior, migration idempotency, add-on lifecycle/authorization/settings foundations, public runtime, CRUD/media cleanup, and rollback without deploying a package or touching client data. |
| Current milestone | The isolated deployment and acceptance gates passed on 2026-08-07 with FrankenPHP 1.12.4, Caddy 2.11.4, and all 45 migrations: HTTPS loaded at desktop and mobile widths with HTTP 200 and zero console/network errors; no opaque token or cookie was disclosed; and the acceptance runner removed its disposable database and grant cleanly. The retained local `redcms_v51_starter` database is an older 20-table snapshot and was intentionally not migrated. Gate 1B is next: review a non-Adriana client deployment boundary, then define the smallest generic richer-enablement/settings surface. Actual settings UI/endpoints, secret lookup, richer package enablement, and Store Lite remain blocked. |
