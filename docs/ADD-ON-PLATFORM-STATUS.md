...
| What is active now? | No required gate remains inside the Store Lite v1 basic-demo target. RED-CMS 5.1.0 is formally released. On the optional hosted-payment path, D4C CLI command and network-disabled no-contact rehearsal is next. D4D remains the separately authorized first real POST. No Stripe key or provider request is authorized. |
