...
The separate 19-assertion creation-preflight fixture permits at most one
...
and exact cleanup. It opens no transaction or endpoint, invokes no creator or
loader, reserves no id, and writes no parent or package row.
...
19-assertion read-only component-creation preflight lifecycle,
...
- Add-on component-creation preflight acceptance runs only in the uniquely named disposable database. It requires declared-editor-only creator registration, exact create permission, enabled manifest and runtime component/loader/creator ownership, closed package-table metadata, InnoDB refusal before callback invocation, an unused numeric record id, one active-theme layout, normalized parent and package values, an inactive hidden unrouted parent plan, deterministic hashing, disabled/revoked/mismatched/invalid/existing-record refusal, unchanged database fingerprints, zero loader/creator invocation, and exact administrator, grant, package, parent, revision, SEO, and table cleanup.
