...
- P3E-9 Sandbox Checkout-creation frontier acceptance is documentation-only. It requires README, roadmap, security, Version 5.1 direction, the P3 proposal, and the canonical status graphic to identify P3E-9A as the next pure non-executing gate; refuse reuse of mutation-disabled P3E-8 evidence; separate synthetic integration, new one-attempt mutation authority, and one real Sandbox Session; and keep credentials, provider POSTs, payment, webhook, browser checkout, demo/client state, and P4 absent. The changed-file credential scan and `git diff --check` must pass, with no PHP, migration, manifest, package, database, route, or runtime changes.
