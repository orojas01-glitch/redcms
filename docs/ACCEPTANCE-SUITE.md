...
24. Compare full checksums for all 35 tables before and after the allowed/denied permission requests and require no data changes.
...
59. Require HTTP 200 with the legacy response `no`, remove the trigger, require the complete 35-table checksum snapshot to match, require the exact initial parent/child values to remain, and require zero updated aliases.
...
- Rollback acceptance requires the legacy `no` response, removal of the trigger, identical pre/post checksums across all 35 tables, exact initial parent/child values, a successful control update after trigger removal, canonical count restoration, and zero targeted database/trigger artifacts.
