...
The separate 132-assertion destination/creation/parent-metadata/public-placement/delete fixture
permits at most one
...
parent/revision drift fail closed. Its destination publication extension then
forces a committed public placement, core `move` revision, and bounded audit to
survive a failed placement checkpoint while the ledger remains at
`component_created`. Retry advances to `component_published` with one placement,
two core revisions, one package baseline, and one audit; exact replay adds
nothing, while package or public-parent drift fails closed. Its terminal search
extension rederives the exact published plan/content, sends only
`component.published` plus the numeric route/component ids through the closed
best-effort index bridge, and records only `completed/succeeded` or
`completed/failed`. It proves contained failure mapping, a successful
notification followed by failed checkpoint, repairable retry, durable success,
completed replay without renotification, unchanged content/revision/audit
counts, and changed-package/placement-drift refusal. Its coordinator extension
then composes a second fresh destination, forces terminal-checkpoint failure
after the first three stages commit, starts retry directly from the retained
`component_published` stage, repeats only repairable Search completion, proves
terminal replay adds no package or Search write, refuses changed immutable
input at completion, and cleans the second route/component/revision/audit/
checkpoint fixture exactly. The parent-metadata
extension requires a fresh
...
- Colombia C4C3A acceptance adds one core durable provider-double helper, one CLI-only dry-run-first command, a 78-assertion source contract, and a network-disabled disposable rehearsal with no migration or package change. Fresh 15-minute authorization evidence binds current client/database/Owner/package/setting/reference/preflight hashes, one nonce, one attempt/no retry, provider-double-only execution, network disabled, and all real provider/business authorizations false. Planning writes nothing. Fully confirmed apply requires all printed hashes plus a nonzero backup hash, commits start/audit before one final core provider double, and records bounded result/audit afterward. The 32-assertion durable test proves exact completion, replay refusal, changed-state refusal, start-audit rollback/recovery, permanently spent post-start failure, and fault/malformed containment. The all-47-migration operator proof passes dry run, incomplete-confirmation refusal, one apply, exactly two nonce rows/two new audits, replay refusal without state change, zero Wompi attempt/event rows, and cleanup `database:0 grant:0 staged-project:0 primary:unchanged`. URL streams and cURL/socket functions are disabled and proxy/secret environments removed. No enrollment/account/tax selection, real key, DNS, TLS, HTTP, Wompi request, provider mutation, transaction, payment, event, order, retained database/client migration, starter package copy, demo, or deployment effect occurs. C4C3B is owner-deferred and requires a new explicit authorization before one real read-only GET.
...
- Read-only public-utility acceptance begins with the dependency-free profile
  ...
  fresh disposable database for all 47 core migrations, exact package
  ...
  The separate 16-assertion two-package rehearsal validates Store Lite 0.1.43
  at 42/42 files and Site Search 0.1.4 at 9/9 files, installs and enables both,
  indexes and finds one real placed Product, refreshes changed text through
  `component.published`, removes an
...
- Commerce-foundation reconciliation pins client kit `0.1.12` to RED-CMS
  ...
  foundation checks. Client kit `0.1.12` also passes archive/internal checksum
  verification and the full current-schema acceptance wrapper with 47 applied
  migrations, a no-op rerun, the complete database/HTTP lifecycle, exact
  disposable database/grant cleanup, and an unchanged configured primary.
  The administrator cart workflow, public review route, authenticated handoff,
  real SDK runtime, signed Sandbox lifecycle, commerce browser QA, and
  deployment remain work for the isolated commerce project.
...
- The Other content lifecycle runs only in the uniquely named disposable database. Its focused assertions require database-derived component identity, exact base64-carried administrator HTML bytes mirrored to both fields, normal and legacy reconciliation writes under a stale-state row lock, exactly one complete pre-change reconciliation revision, restore of the former mismatch, unchanged Article field independence, unchanged metadata/placement/image-only content, identical embedded and dedicated rendering, rollback, and endpoint CSRF/authorization contracts.
