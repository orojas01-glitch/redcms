...
The separate 44-assertion transactional update/restore fixture permits at most one
...
The separate 31-assertion creation-preflight/runner fixture permits at most one
creator for a declared editor with closed package-table metadata. It requires
the exact create grant, enabled manifest and runtime component/loader/creator
...
59. Require HTTP 200 with the legacy response `no`, remove the trigger, require the complete 27-table checksum snapshot to match, require the exact initial parent/child values to remain, and require zero updated aliases.
...
44-assertion transactional component-editor update, history, restore-preflight,
and atomic-restore lifecycle,
31-assertion component-creation preflight/atomic-runner lifecycle,
...
- Add-on component-creation preflight/runner acceptance runs only in the uniquely named disposable database. It requires declared-editor-only creator registration, exact create permission, enabled manifest and runtime component/loader/creator ownership, closed package-table metadata, InnoDB refusal before callback invocation, an unused numeric record id, one active-theme layout, normalized parent and package values, an inactive hidden unrouted parent plan, deterministic hashing, disabled/revoked/mismatched/invalid/existing-record refusal, and zero loader/creator invocation during preflight. The atomic runner requires exact plan matching under lifecycle/theme/installation serialization, rejects caller-owned transactions, contains creator/loader failures, verifies exact parent and package postconditions, commits one core `create` and one package `baseline` revision, refuses reuse, rolls back output/exception/nested-buffer/false/partial-write and both forced-ledger failures, and leaves zero administrator, grant, package, parent, revision, SEO, or table fixtures.
