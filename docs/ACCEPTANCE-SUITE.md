...
body handling. Direct rendered Chrome desktop/mobile evidence remains pending
when desktop browser launch permission is unavailable.
...
- Add-on administrator-tool form current-value acceptance runs only in the uniquely named disposable database with one temporary package-owned InnoDB table and an in-memory trusted runtime context. It requires one exact schema-bearing form loader owner, fresh binary package permission independent of Owner/lifecycle authority, complete simple/variable and nested option values, deterministic target/contract-bound state evidence, escaped disabled nameless current-value markup, next-decision revocation, invalid/extra/oversized value refusal, output/exception/buffer containment, unchanged package data, and exact administrator/grant/table cleanup. The provider receives no actor or request/session value, and the boundary creates no editable control, request parser, CSRF operation, endpoint, write, package, or Store Lite state.
...
- Add-on administrator-tool form Save-bridge acceptance runs only in the uniquely named disposable database with one temporary package-owned InnoDB table, enabled installation, explicit package grant, and in-memory trusted runtime context. It requires exact tool/form/canonical-positive-target edit identity, fresh permission and enabled-version checks, exact loader/writer/table ownership, complete current values, escaped core-only editable scalar and two-level collection markup, bounded add/remove templates, typed canonical JSON with header CSRF, an atomic value-free `saved` result, no-op `unchanged`, stale replay and next-decision revocation refusal, and exact administrator/role/grant/package/audit/table cleanup. Source and real HTTP checks require POST, database-backed session, and header CSRF before edit parsing or Save body I/O; public responses disclose no values, state/plan evidence, package identity, or tables. Direct rendered desktop/mobile browser inspection remains a separately reported requirement when Chrome launch is unavailable.
