...
- Add-on package-permission mutation acceptance runs only in the uniquely named disposable database and a temporary integrity-valid package. Its 15 assertions require non-executing manifest discovery, a fresh Owner, exact declared permission and target checks, deterministic zero-write planning, stale-plan and caller-transaction refusal, serialized atomic grant and revoke, one permission-specific actor/target audit fact per change, immediate revocation, no package callback execution, and exact administrator/grant/audit/filesystem cleanup.
