...
if (red_admin_form_record_exists($db->connection, $recordId) && !red_admin_form_record_matches($db->connection, $recordId, $artRecordId)) {
	echo 'no';
	$db->close();
	exit;
}
...
if (red_admin_form_uses_registration_table($formData['FormType'] ?? '') && ($formData['TableName'] ?? '') !== '') {
	if (!red_admin_form_create_registration_table($db->connection, $formData['TableName'], $formData['LongDesc'] ?? '')) {
		echo 'no';
		$db->close();
		exit;
	}
...
$success = red_admin_write_transaction($connection, function () use ($connection, $existingArticle, $artRecordId, $articleData, $recordId, $formData) {
	if ($existingArticle) {
