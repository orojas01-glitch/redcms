...
if ($title === '' || $subcategories === '' || $language === '') {
    echo 'no';
    $db->close();
    exit;
}
...
if (red_admin_insert_area($db->connection, 'RED_SubCategories', 'SubCategories', $title, $subcategories, $layout, $queryLimit, $accessLevel, $features, $active, $description, $tags, $language)) {
    echo 'yes';
} else {
    echo 'no';
}
