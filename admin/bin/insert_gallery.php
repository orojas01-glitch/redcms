...
if (red_admin_gallery_record_exists($db->connection, $recordId) && !red_admin_gallery_record_matches($db->connection, $recordId, $artRecordId)) {
	echo 'no';
	$db->close();
	exit;
}

$articlePost = red_admin_gallery_article_post($_POST, $artRecordId, 'insert');
...
$galleryData = red_admin_gallery_collect_values($_POST, 'insert', $recordId, $artRecordId);
$connection = $db->connection;
$success = red_admin_write_transaction($connection, function () use ($connection, $existingArticle, $artRecordId, $articleData, $recordId, $galleryData) {
	if ($existingArticle) {
