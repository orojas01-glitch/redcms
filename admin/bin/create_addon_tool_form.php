<?php
/**
 * Protected JSON Create bridge for one manifest-declared add-on tool form.
 */
...
try {
    ...
    $result = red_addon_admin_tool_form_create_dispatch(
        $db->connection,
        $transport['rawBody'],
        (int) ($_SESSION['AdminRecordID'] ?? 0)
    );
} catch (Throwable $throwable) {
    error_log('RED-CMS administrator form Create endpoint failed.');
    $result = red_addon_admin_tool_form_create_failure(
